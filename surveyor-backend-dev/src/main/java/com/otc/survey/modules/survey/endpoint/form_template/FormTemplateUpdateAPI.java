...
@APIController
@RequestMapping("/api/survey/form-template/setup/update")
@Auth(
	authen = true,
	authorize = true,
	grantedRoleIds = {
...
public class FormTemplateUpdateAPI extends AbstractJsonBodyAPI<FormTemplateUpdateAPI.RequestMessage, FormTemplateUpdateAPI.BodyResponseMessage>
{
	@Autowired
	...
	protected ResponseEntity<ResponseMessage<BodyResponseMessage>> doExecute(HttpServletRequest request, HttpServletResponse response, RequestMessage requestMessage) 
	{
		...
		FormTemplateUpdateDTO.RequestPayload requestPayload = requestMessage.getRequestPayload();
		...
		try {
			FormTemplateUpdateDTO.ResponsePayload responsePayload = formTemplateService.updateFormTemplate(req -> {
