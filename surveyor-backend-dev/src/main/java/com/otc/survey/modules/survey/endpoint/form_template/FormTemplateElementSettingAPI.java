...
@APIController
@RequestMapping("/api/survey/form-template/setup/element-setting")
@Auth(
	authen = true,
	authorize = true,
	grantedRoleIds = {
...
public class FormTemplateElementSettingAPI extends AbstractJsonBodyAPI<FormTemplateElementSettingAPI.RequestMessage, FormTemplateElementSettingAPI.BodyResponseMessage>
{
	@Autowired
	...
	protected ResponseEntity<ResponseMessage<BodyResponseMessage>> doExecute(HttpServletRequest request, HttpServletResponse response, RequestMessage requestMessage) 
	{
		...
		FormTemplateElementSettingDTO.RequestPayload requestPayload = requestMessage.getRequestPayload();
		...
		try {
			FormTemplateElementSettingDTO.ResponsePayload responsePayload = formTemplateService.setElement(req -> {
		...
		} catch(FormTemplateElementSettingException ex) {
