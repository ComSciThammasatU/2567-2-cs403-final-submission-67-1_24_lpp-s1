...
@ServiceCommand
public class LoadStudentInfoCommand extends BaseServiceCommand
{
	@Autowired
	...
	public Response loadStudentInfo(Request request) throws LoadStudentInfoException
	{
		logger.info("### {}.loadStudentInfo ###", getClass().getSimpleName());
		
		if(request.getStudentId() == null || request.getStudentId().isBlank()) {
			throw new IllegalArgumentException("Param 'studentId' couldn't be null or empty");
		}
		
		Student student = studentRepository.findById(request.getStudentId()).orElse(null);
		
		if(student == null) {
			throw new LoadStudentInfoException.StudentNotFound(request.getStudentId());
		}
		
		User user = userRepository.findById(student.getUserId()).orElse(null);
		
		if(user == null) {
			throw new LoadStudentInfoException.UserNotFound(student.getUserId());
		}
		
		StudentInfo studentInfo = new StudentInfo();
		studentInfo.setStudent(student);
		studentInfo.setUser(user);
		studentInfo.setRoles(loadRoles(userRoleMapperRepository.findByUserId(user.getId())));
		studentInfo.setUserGroups(loadUserGroups(userGroupMapperRepository.findByUserId(user.getId())));
