...
@APIController
@RequestMapping("/api/core/staff/setup/load-info")
@Auth(
	authen = true,
	authorize = true,
	grantedRoleIds = {
...
public class StaffSetupLoadStaffInfoAPI extends AbstractJsonBodyAPI<StaffSetupLoadStaffInfoAPI.RequestMessage, StaffSetupLoadStaffInfoAPI.BodyResponseMessage>
{
	@Autowired
	...
	protected ResponseEntity<ResponseMessage<BodyResponseMessage>> doExecute(HttpServletRequest request, HttpServletResponse response, RequestMessage requestMessage) 
	{
		...
		if(requestMessage.getStaffId() == null || requestMessage.getStaffId().isBlank()) {
			return replyError(request, "400002", "Invalid Request Message", "Param 'studentId' Couldn't Be Null OR Empty");
		}
		
		try {
			StaffInfo staffInfo = staffService.loadStaffInfo(req -> {
				setupServiceRequest(request, req);
				req.setStaffId(requestMessage.getStaffId().trim());
			}).getStaffInfo();
		...
		} catch (LoadStaffInfoException ex) {
			logger.error(ex.getMessage(), ex);
			
			if(ex instanceof LoadStaffInfoException.UserNotFound) {
